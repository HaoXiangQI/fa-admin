...
from app.db.migrations.inspectdb import InspectDb
from app.db.migrations.migrate import Migrate
from app.db.migrations.utils import (
    get_app_connection,
...
from app.db.migrations import __version__
from app.db.migrations.enums import Color
from app.db.migrations.models import Aerich

# todo 将init和init-db合并
...
@click.pass_context
@coro
async def migration_cli(ctx: Context, config, app, name):
...
@migration_cli.command(help="Generate migrate changes file.")
...
@migration_cli.command(help="Upgrade to specified version.")
@click.pass_context
@coro
...
@migration_cli.command(help="Downgrade to specified version.")
@click.option(
    "-v",
    "--version",
    default=-1,
...
@migration_cli.command(help="Show current available heads in migrate location.")
@click.pass_context
@coro
...
@migration_cli.command(help="List all migrate items.")
@click.pass_context
@coro
...
@migration_cli.command(help="Init config file and generate root migrate location.")
@click.option(
    "--location",
    default="./migrations",
    show_default=True,
...
@migration_cli.command(help="Generate schema and generate app migrate location.")
@click.option(
    "--safe",
    type=bool,
    default=True,
...
@migration_cli.command(help="Introspects the database tables to standard output as TortoiseORM model.")
@click.option(
    "-t",
    "--table",
